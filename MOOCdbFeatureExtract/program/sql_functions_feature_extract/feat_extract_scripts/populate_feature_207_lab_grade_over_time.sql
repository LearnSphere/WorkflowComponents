...
CREATE PROCEDURE `moocdb`.populate_207()
    BEGIN
        ...
        WHILE x  < v_max DO
            ...
            SELECT @feature_extraction_id, 207, d1.user_id, x AS week, d1.longitudinal_feature_value -
                (SELECT AVG(longitudinal_feature_value)
                FROM `moocdb`.user_longitudinal_feature_values AS d2 WHERE feature_extraction_id = @feature_extraction_id AND longitudinal_feature_id = 206 AND longitudinal_feature_week < x AND d1.user_id = d2.user_id ),
                @current_date
            FROM `moocdb`.user_longitudinal_feature_values AS d1 WHERE feature_extraction_id = @feature_extraction_id AND longitudinal_feature_id = 206 AND longitudinal_feature_week = x ;
            SET  x = x + 1;
        END WHILE;
    END;
