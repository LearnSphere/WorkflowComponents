...
while (i <= length(args)) {
  if (args[i] == "-node") {
    ...
    else if (args[i] == "-sparfalite") {
    if (length(args) == i) {
      stop("sparfalite must be specified")
    }
    sparfalite = args[i+1]
    i = i+1
  }
...
#parameters
sparfalite<-as.numeric(sparfalite)
...
#==========================Reduce matrix================================
reducedmatrix<-rsvd(t(Z$z),RSVDcomp)
...
switch(Sys.info()[['sysname']],
Linux  = { bitmap(file = paste(workingDirectory, "graphAM.png", sep=""),"png16m") },
#Linux  = { bitmap(file = paste(workingDirectory, "graphAM.png", sep=""),type="png16m",width=1000, height=800, res=300)},
Windows= { png(file = paste(workingDirectory, "graphAM.png", sep=""), width=1000, height=800, res=300) },
Darwin = { png(file = paste(workingDirectory, "graphAM.png", sep=""), width=1000, height=800, res=300) })
