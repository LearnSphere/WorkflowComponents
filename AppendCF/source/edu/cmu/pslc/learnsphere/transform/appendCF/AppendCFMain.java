...
public class AppendCFMain extends AbstractComponent {
    ...
    protected void runComponent() {
        ...
        Boolean reqInputsExist = true;

        if (file1 == null || file2 == null) {
            addErrorMessage("The Transform -> AppendCF component requires two input files.");
            reqInputsExist = false;
        }

        if (!(isStudentStepFile(file1Type) && isTransactionFile(file2Type)) &&
                !(isStudentStepFile(file2Type) && isTransactionFile(file1Type))){
            addErrorMessage("The Transform -> AppendCF component requires one transaction and one student-step file.");
            reqInputsExist = false;
        }
