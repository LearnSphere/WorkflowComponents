        ...
        print(paste("fold " , fold))
            ...
            decmod <- function(tem) {
              ...
              fitmodel <<- glm(CF..ansbin.~ 
                          log((t1+CF..cor.)/(t1+CF..incor.))+log((t1+CF..czcor.)/(t1+CF..czincor.))+log((s+CF..totcor.)/(s+CF..totincor.))+
              ...
              -logLik(fitmodel)[1]}
            fitoptim <- optim(c(.4,.05,.05,1,5),decmod,method = c("L-BFGS-B"),lower = .001, upper = 2, control = list(maxit = 250))
