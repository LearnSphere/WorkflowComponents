...
CF..meanspacingval.<-0
terms<- c("I(CF..cor.-CF..incor.)",
          "I(CF..clcor.-CF..clincor.)",
          "I(CF..czcor.-CF..czincor.)",
          "I(CF..totcor.-CF..totincor.)",
          "I(CF..baselevel.*log(CF..tests.+1)*ifelse(CF..meanspacingval.==1,w,((CF..meanspacingval.)^k)))",
          "I(CF..baselevel.*log(1+CF..clcor.+CF..clincor.))",
...
for(v in 1:length(terms)){
  composedform <- paste(composedform,ifelse(vec[v],paste(terms[v],"+",sep=""),
                                            ifelse(pars[v]!=0,paste("offset(I(",pars[v],"*",terms[v],"))+",sep=""),"")),sep="")}
       ...
       "best fit model" = {
         ...
         if(all(vec[8:10]==c(0,1,0))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.05,.5),decmod,method = c("L-BFGS-B"),lower = .001, upper = .7, control = list(maxit = 1000))
           pr<-2}
         ...
         if(all(vec[8:10]==c(1,1,0))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.4,.05,.5),decmod,method = c("L-BFGS-B"),lower = .001, upper = .7, control = list(maxit = 1000))
           pr<-3}
         ...
         if(all(vec[8:10]==c(0,1,1))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.05,.05,.5),decmod,method = c("L-BFGS-B"),lower = .001, upper = .7, control = list(maxit = 1000))
           pr<-3}
         ...
         if(all(vec[8:10]==c(1,1,1))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.4,.05,.05,.5),decmod,method = c("L-BFGS-B"),lower = .001, upper = .7, control = list(maxit = 1000))
           pr<-4}
         
         #Output text summary
         names(x$coefficients)<-substr(names(x$coefficients),1,75)
         print(summary(x))
         print(c("decay rate",j,"spacing effect",k,"interference rate",f,"spacing effect surrogate for trial 2",w))
           ...
           for(fold in 1:2){
             ...
             if(all(vec[8:10]==c(0,1,0))){
               decmod <- function(tem) {
                 ...
                 fitmodel <<- glm(as.formula(composedform),data=trainfold,family=binomial(logit))
                 -logLik(fitmodel)[1]}
               fitoptim<-optim(c(.05,.5),decmod,method = c("L-BFGS-B"),lower = .001, upper = .7, control = list(maxit = 1000))
               pr<-2}
             ...
             if(all(vec[8:10]==c(1,1,0))){
               decmod <- function(tem) {
                 ...
                 fitmodel <<- glm(as.formula(composedform),data=trainfold,family=binomial(logit))
                 -logLik(fitmodel)[1]}
               fitoptim<-optim(c(.4,.05,.5),decmod,method = c("L-BFGS-B"),lower = .001, upper = .7, control = list(maxit = 1000))
               pr<-3}
             ...
             if(all(vec[8:10]==c(0,1,1))){
               decmod <- function(tem) {
                 ...
                 fitmodel <<- glm(as.formula(composedform),data=trainfold,family=binomial(logit))
                 -logLik(fitmodel)[1]}
               fitoptim<-optim(c(.05,.05,.5),decmod,method = c("L-BFGS-B"),lower = .001, upper = .7, control = list(maxit = 1000))
               pr<-3}
