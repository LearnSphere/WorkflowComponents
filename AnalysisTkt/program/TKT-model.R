       ...
       "best fit model" = {
         ...
         if(all(vec[8:10]==c(1,0,0))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.3),decmod,method = c("L-BFGS-B"),lower = .0001, upper = .7, control = list(maxit = 1000))
           pr<-1}
         ...
         if(all(vec[8:10]==c(0,1,0))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.3,.3),decmod,method = c("L-BFGS-B"),lower = .0001, upper = .7, control = list(maxit = 1000))
           pr<-2}
         ...
         if(all(vec[8:10]==c(1,1,0))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.3,.3,.3),decmod,method = c("L-BFGS-B"),lower = .0001, upper = .7, control = list(maxit = 1000))
           pr<-3}
         ...
         if(all(vec[8:10]==c(1,1,1))){
           decmod <- function(tem) {
             ...
             x <<- glm(as.formula(composedform),data=dat,family=binomial(logit))
             -logLik(x)[1]}
           optim(c(.3,.3,.3,.3),decmod,method = c("L-BFGS-B"),lower = .0001, upper = .7, control = list(maxit = 1000))
           pr<-4}
           ...
           for(fold in 1:2){
             ...
             if(all(vec[8:10]==c(1,0,0))){
               decmod <- function(tem) {
                 ...
                 fitmodel <<- glm(as.formula(composedform),data=trainfold,family=binomial(logit))
                 -logLik(fitmodel)[1]}
               fitoptim<-optim(c(.3),decmod,method = c("L-BFGS-B"),lower = .0001, upper = .7, control = list(maxit = 1000))
               pr<-1}
             ...
             if(all(vec[8:10]==c(0,1,0))){
               decmod <- function(tem) {
                 ...
                 fitmodel <<- glm(as.formula(composedform),data=trainfold,family=binomial(logit))
                 -logLik(fitmodel)[1]}
               fitoptim<-optim(c(.3,.3),decmod,method = c("L-BFGS-B"),lower = .0001, upper = .7, control = list(maxit = 1000))
               pr<-2}
             ...
             if(all(vec[8:10]==c(1,1,0))){
               decmod <- function(tem) {
                 ...
                 fitmodel <<- glm(as.formula(composedform),data=trainfold,family=binomial(logit))
                 -logLik(fitmodel)[1]}
               fitoptim<-optim(c(.3,.3,.3),decmod,method = c("L-BFGS-B"),lower = .0001, upper = .7, control = list(maxit = 1000))
               pr<-3}
