...
def restoreDB (vars, dbName, sourceFileName, dbHost):
    if dbHost == "127.0.0.1":
        ...
        output = subprocess.call(['mysql', dbName, '--host={}'.format(dbHost), '--user={}'.format(vars['source']['user']), '--password={}'.format(vars['source']['password']), '-e', 'source {}'.format(sourceFileName)])
    vars['logger'].Log(vars, "Restored database: {}; on host: {}; source SQL file: {}; output: {}.".format(dbName, dbHost, sourceFileName, output))
