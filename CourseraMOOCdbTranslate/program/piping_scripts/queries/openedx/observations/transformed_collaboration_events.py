...
def GetTransformedCollaborationEvents(vars):
    ...
    rows = db.Select(vars['curs']['target'], "SELECT * FROM collaborations JOIN collaboration_types ON collaborations.collaboration_type_id=collaboration_types.collaboration_type_id")
    for row in rows:
