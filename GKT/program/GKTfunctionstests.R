  ...
  tempfun <- function(pars){
    ...
    if(any(grep("[@]",feats)) & dualfit==FALSE){
      temp<<-glmer(as.formula(paste(equation,eq,sep="")),data=df,family=binomial(logit))
        fitstat<<- cor(temp@frame$CF..ansbin.,predict(temp,type="response"))^2#-sqrt(mean((temp@frame$CF..ansbin.-predict(temp,type="response"))^2))
        } else if(dualfit==FALSE){
        temp<<-glm(as.formula(paste(equation,eq,sep="")),data=df,family=binomial(logit),x=TRUE)
        fitstat<<- cor(temp$data$CF..ansbin.,predict(temp,type="response"))^2#-sqrt(mean((temp$data$CF..ansbin.-predict(temp,type="response"))^2))
      }
    #print("statnext")
