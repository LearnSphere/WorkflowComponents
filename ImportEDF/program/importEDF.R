...
if (is.null(inputFile) || is.null(signal) || is.null(workingDirectory) || is.null(componentDirectory)) {
  if (is.null(inputFile)) {
    warning("Missing required input parameter: -file0")
  }
if (is.null(signal)) {
    warning("Missing required input parameter: -signal")
  }
...
clean <- file(paste(workingDirectory, "Import-EDF-log.txt", sep=""))
...
d = read.edf(inputFile, read.annotations = TRUE, header.only = FALSE )

if (signal =="EEG_Fpz_Cz"){

t = d$signal$EEG_Fpz_Cz$t
data =d$signal$EEG_Fpz_Cz$data
sample_rate=d$header.signal$EEG_Fpz_Cz$n.samples
} else if(signal =="EEG_Pz_Oz"){

t = d$signal$EEG_Pz_Oz$t
data =d$signal$EEG_Pz_Oz$data
sample_rate=d$header.signal$EEG_Pz_Oz$n.samples

} else if(signal =="EOG_horizontal"){

t = d$signal$EOG_horizontal$t
data =d$signal$EOG_horizontal$data
sample_rate=d$header.signal$EOG_horizontal$n.samples

} else if(signal =="Resp_oro_nasal"){

t = d$signal$Resp_oro_nasal$t
data =d$signal$Resp_oro_nasal$data
sample_rate=d$header.signal$Resp_oro_nasal$n.samples

} else if(signal =="EMG_submental"){

t = d$signal$EMG_submental$t
data =d$signal$EMG_submental$data
sample_rate=d$header.signal$EMG_submental$n.samples

} else if(signal =="Temp_rectal"){
print(signal)
t = d$signal$Temp_rectal$t
data =d$signal$Temp_rectal$data
sample_rate=d$header.signal$Temp_rectal$n.samples

} else if (signal =="Event_marker"){

t = d$signal$Event_marker$t
data =d$signal$Event_marker$data
sample_rate=d$header.signal$Event_marker$n.samples
...
write.table(n,file=paste(workingDirectory, "Results.txt", sep=""),sep="\t",quote=FALSE,na = "NA",append=FALSE,col.names=TRUE,row.names = FALSE)
