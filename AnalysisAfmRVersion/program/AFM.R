#"C:/Program Files/R/R-3.4.1/bin/Rscript.exe" AFM_actR.R -programDir . -workingDir . -userId hcheng -kcmodel_nodeIndex 0 -kcmodel_fileIndex 0 -kcmodel "KC (Circle-Collapse)" -reverseOpportunity "No" -paramOutputFormat "Tab-delimited" -node 0 -fileIndex 0 ds76_student_step_export.txt
...
preprocess <- function(origRollup, kcm) {
  ...
  df$opportunity_reverse = as.numeric(df$opportunity_reverse)
  return(df)
}
...
while (i <= length(args)) {
  if (args[i] == "-node") {
    ...
    modelName = args[i+1]
    i = i+1
  } else if (args[i] == "-reverseOpportunity") {
    if (length(args) == i) {
      stop("includeDecay name must be specified")
    }
    reverseOpportunity = args[i+1]
    i = i+1
  } 
  i = i+1
}
...
if (!reverseOpportunity) {
  ...
  model <- logWarningsMessages(glmer(response ~ opportunity_reverse + (opportunity_reverse|KC) + (1|individual), data=df, family=binomial(), nAGQ = 0), logFileName = wfl_log_file)
}
...
if(reverseOpportunity) {
  #add opportunity_reverse to output
  origFile <- cbind(origFile, df$opportunity_reverse)
  names(origFile)[ncol(origFile)] <- 'opportunity_reverse'
}
