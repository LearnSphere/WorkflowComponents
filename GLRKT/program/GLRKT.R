...
#Help Functions "computefeatures","baselevel","countOutcome","right","slidepropdec","propdec",""
...
baselevel <-  function(x, d) {
  return(c(0,x[2:length(x)]^-d))}#end baselevel
...
switch(mode,
       "best fit model"={
        ...
        #print(seeds)
          ...
          R1<-r.squaredLR(temp)
          r2LR<-round(R1[1],5)
          r2NG<-round(attr(R1,"adj.r.squared"),5)
          #R2<-r.squaredGLMM(temp)
          #glmmR2fixed<-round(R2[1],5)
          #glmmR2random<-round(R2[2]-R2[1],5)
         ...
         newXMLNode("r2LR", r2LR , parent = top)  
         newXMLNode("r2NG", r2NG, parent = top)               
        ...
        #Create Function GLRKT.cv
         ...
         print(paste("run",i))
          ...
          #Output text summary
                ...
                newXMLNode("RMSE", round(sqrt(mean((predfit-dat$CF..ansbin.)^2)),5), parent = d)
                newXMLNode("Accuracy", round(sum(dat$CF..ansbin.==(predfit>.5))/Nresfit,5), parent = d)
                ...
                newXMLNode("tRMSE", round(sqrt(mean((predtest-dat$CF..ansbin.)^2)),5), parent = d)
                newXMLNode("tAccuracy", round(sum(dat$CF..ansbin.==(predtest>.5))/Nrestest,5), parent = d)
