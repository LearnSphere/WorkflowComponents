...
//
ScatterMatrix = function(url, data, dom_id, el, file_form, control_id) {
    this.__url = url;
    if (file_form === undefined) {
        this.__file_form = "csv";
    } else {
        this.__file_form = file_form;
    }
    if (data === undefined || data === null) {
        this.__data = undefined;
    } else {
        if (this.__file_form === "tsv") {
            this.__data = d3.tsv.parse(data);
        } else if (this.__file_form === "json") {
            this.__data = d3.json(this.__url);
        } else {
            this.__data = d3.csv.parse(data);
        }
    }
    ...
    if (control_id === undefined) {
        this.__control_id = this.__dom_id;
    } else {
        if (control_id[0] !== "#") {
            control_id = "#" + control_id;
        }
        this.__control_id = control_id;
    }
};
...
ScatterMatrix.prototype.onData = function(cb) {
    ...
    var self = this;
    if (this.__file_form === "tsv") {
        d3.tsv(self.__url).then(function(data){
          self.__data = data;
          cb();
        });
        ...
        d3.json(self.__url).then(function(data){
          self.__data = data;
          cbn();
        });
        // d3.json(self.__url, function(data) {
        //     self.__data = data;
        //     cb();
        // });
    } else {
        d3.csvParse(self.__url).then(function(data){
          self.__data = data;
          cb();
        });
...
ScatterMatrix.prototype._numeric_to_str_key = function(k) {
    return k + '_';
};
ScatterMatrix.prototype._is_numeric_str_key = function(k) {
    return k[k.length - 1] === '_';
};
ScatterMatrix.prototype._str_to_numeric_key = function(k) {
    if (this._is_numeric_str_key(k)) {
        return k.slice(0, k.length - 1);
    }
    return null;
};
ScatterMatrix.prototype._color_value = function(k,v){
  return this.__color_scales[k].scale(v);
}
...
ScatterMatrix.prototype.render = function() {
    ...
    var control = d3.select(this.__control_id).append('ul')
    ...
    // var svg = d3.select(this.__dom_id);
    ...
    this.onData(function() {
        ...
        for (var j in string_variables) {
            if (string_variables[j].length <= 10) {
              this.__color_scales[j] = d3.scaleOrdinal(d3.schemeCategory10).domain(string_variables[j]);
            }
            else {
              this.__color_scales[j] = d3.scaleOrdinal(d3.schemeSet3).domain(string_variables[j]); 
            }
        }
        // data.forEach(function(d) {
        //     for (var j in string_variables) {
        ...
        var color_control = control.append('li');//.attr('class', 'nav-item');
        var filter_control = control.append('li');//.attr('class', 'nav-item');
        var variable_control = control.append('li');//.attr('class', 'nav-item');
        var drill_control = control.append('li');//.attr('class', 'nav-item');
        var size_control = control.append('li');//.attr('class', 'nav-item');
        ...
        function set_filter(variable) {
            ...
            if (variable) {
                ...
                filter_control.append('a')
                    .attr('data-toggle', 'collapse')
                    .attr('href', '#filter-collapse')
                    .text('Filter by ' + variable + ': ')
                var filter_li = filter_control.append('div')
                    .attr('class', 'collapse')
        ...
        size_a = size_control.append('div')
            .attr('class', 'btn-toolbar mb-2 mb-md-0')
            .append('div')
        ...
        color_control.append('a')
            .attr('class', 'nav-link')
            .attr('data-toggle', 'collapse')
            .attr('href', '#colorby-collapse')
            .text('Color By')
        color_control.append('div')
            .attr('class', 'collapse')
            .attr('id', 'colorby-collapse')
            .append('ul')
            .selectAll('li')
        ...
        variable_control.append('a')
            .attr('class', 'nav-link')
            .attr('data-toggle', 'collapse')
            .attr('href', '#include-collapse')
            .text('Include')
        var variable_li = variable_control.append('div')
            .attr('class', 'collapse')
            .attr('id', 'include-collapse')
            .append('ul')
            .selectAll('li')
        ...
        drill_control.append('a')
            .attr('class', 'nav-link')
            .attr('data-toggle', 'collapse')
            .attr('href', '#drill-collapse')
            .text('Drill and Expand')
        var drill_li = drill_control.append('div')
            .attr('class', 'collapse')
            .attr('id', 'drill-collapse')
            .append('ul')
            .selectAll('li')
    ...
    function(cell_size, container_el, color_variable, selected_colors, to_include, drill_variables) {
        ...
        this.onData(function() {
            ...
            variables_to_draw.forEach(function(trait) {
                ...
                x[trait] = d3.scaleLinear().domain(domain).range(range_x);
                y[trait] = d3.scaleLinear().domain(domain).range(range_y.reverse());
            });
            ...
            var reshape_axis = function(axis, k) {
                if (self._is_numeric_str_key(k)) {
                    ...
                    axis.tickFormat(function(d) {
                        return self.__string_variable_values[sk][d];
                    });
            ...
            svg.selectAll("g.x.axis")
                ...
                .each(function(k) {
                    var axis = reshape_axis(d3.axisBottom(), k);
            ...
            svg.selectAll("g.y.axis")
                ...
                .each(function(k) {
                    var axis = reshape_axis(d3.axisRight(), k);
            ...
            function plot(p) {
                ...
                // cell.call(brush.extent([[x[p.x].range()[0], y[p.y].range()[0]], [x[p.x].range()[1], y[p.y].range()[1]]]));
                cell.call(brush.extent([[x[p.x].range()[0], 0], [x[p.x].range()[1], y[p.y].range()[1]]]));
            }
            ...
            function brushstart(p) {
                ...
                if (brush.data !== p) {
                    ...
                    // brush.extent([[xScale.range()[0], 0], [xScale.range()[1], brushHeight]])
                    // brush.x(x[p.x]).y(y[p.y]).data = p;
                    brush.extent([[x[p.x].range()[0], 0], [x[p.x].range()[1], y[p.y].range()[1]]]).data=p;
