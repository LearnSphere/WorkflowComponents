...
public class LDAUtil
{
    ...
    public LDAUtil() throws Exception {
    }
    ...
    public void loadSpace(String path) throws Exception {
        ...
        */
        if ((new File(path + "/model-final-phi.txt")).exists() && ((new File(path + "/wordmap.txt")).exists()))
        {
            this.loadMatrix_lda(path);
            this.loadTerms_lda(path);
        }
        else
        {
	...
	private void loadMatrix_lda(String path) throws Exception {
		this.vectors = null;

		int[] fileSize = LineCount(path + "\\model-final-phi.txt");
		this.dim = fileSize[0];
		this.vocab = fileSize[1];
		this.vectors = new float[dim][vocab];
	...
	private void loadTerms_lda(String path) throws Exception {
		FileReader fs2 = new FileReader(path + "\\wordmap.txt");
		BufferedReader sr2 = new BufferedReader(fs2);
		List<String> AllLines = new ArrayList<String>();
		String line = null;
		sr2.readLine(); // excluding first line
		while ((line = sr2.readLine()) != null) {

			AllLines.add(line.split(" ")[0]);
		}
    ...
    private float[] getVector(String[] tokens) throws Exception {
        ...
        for (Object __dummyForeachVar0 : wordFreq.keySet())
        {
            ...
            for (int i = 0;i < this.dim;i++)
                ret[i] = ret[i] + (float)(logf * ent * this.vectors[i][index]);
        }
        return ret;
    }
