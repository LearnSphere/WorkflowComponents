...
public class RowOperationsMain extends AbstractComponent {
  ...
  private void addErrorsAndDebugsToLogger(String outputPath) {
    ...
    for (int i = 0; i < errors.size(); i++) {
      errorMessages.add("[error from RowOperations.java] " + errors.get(i));
    }

    ArrayList<String> debugMessages = getDebugMessagesFromComponent(outputPath);
    for (int i = 0; i < debugMessages.size(); i++) {
      logger.debug("[debug from RowOperations.java] " + debugMessages.get(i));
    }
