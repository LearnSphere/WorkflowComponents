#"C:/Program Files/R/R-3.4.1/bin/Rscript.exe" AFM_actR.R -programDir . -workingDir . -userId hcheng -kcmodel_nodeIndex 0 -kcmodel_fileIndex 0 -kcmodel "KC (Circle-Collapse)" -node 0 -fileIndex 0 ds76_student_step_export.txt
...
model <- logWarningsMessages(glmer(response ~ cumulative.corrects + cumulative.incorrects + (cumulative.corrects + cumulative.incorrects |KC) + (1|individual), data=df, family=binomial(), nAGQ = 0 ), logFileName = wfl_log_file)
